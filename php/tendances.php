...
function allTendance($bd){
	...
	echo '<div id=tendance>',
			...
			while($TJ=mysqli_fetch_assoc($Topjour)){
				...
				echo
				'<li class="tend"><a href="tendances.php?tags=',cryptage($tag),'">',$tag,'(',wa_html_proteger_sortie($TJ['NB']),')','</a></li>';
				$exist=false;
			}
			if($exist){
				echo
	...
	echo
			...
			while($TS=mysqli_fetch_assoc($Topsemaine)){
				...
				echo
				'<li class="tend"><a href="tendances.php?tags=',cryptage($tag),'">',$tag,'(',wa_html_proteger_sortie($TS['NB']),')','</a></li>';
				$exist=false;
			}
			if($exist){
				echo
			...
			while($TM=mysqli_fetch_assoc($Topmois)){
				...
				echo
				'<li class="tend"><a href="tendances.php?tags=',cryptage($tag),'">',$tag,'(',wa_html_proteger_sortie($TM['NB']),')','</a></li>';
				$exist=false;
			}
			if($exist){
				echo
	...
	echo
			'</ol>',
			'<h2 id="annee">Top 10 de l\'année</h2>',
				'<ol>';
			$exist=true;
			while($TA=mysqli_fetch_assoc($Topannee)){
				$tag=wa_html_proteger_sortie($TA['taID']);
				echo
				'<li class="tend"><a href="tendances.php?tags=',cryptage($tag),'">',$tag,'(',wa_html_proteger_sortie($TA['NB']),')','</a></li>';
				$exist=false;
			}
			if($exist){
				echo
...
if (!isset($_GET['tags'])) {
	...
	if ($nombrebl==0) {
		...
		echo '<ul>';
		wa_aff_blablas($bd, $blabla, $nombrebl);
		echo '</ul>';
	}
}
