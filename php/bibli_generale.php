 ...
 */
function wa_bd_erreur_exit(array $err):void {
...
function wa_bd_connect(): mysqli {
    ...
    try{
        ...
        $err['autres'] = array('Paramètres' =>   'BD_SERVER : '. BD_SERVER
                                                    ...
                                                    ."\n".'BD_PASS : '. BD_PASS
                                                    ."\n".'BD_NAME : '. BD_NAME);
        wa_bd_erreur_exit($err); // ==> ARRET DU SCRIPT
    }
    try{
 ...
 */
function wa_bd_send_request(mysqli $bd, string $sql): mysqli_result|bool {
    try{
 ...
 */
function wa_aff_debut(string $titre, ?string $css = null):void {
...
*/
function wa_amj_clair(int $amj):string {
...
*/
function wa_heure_clair(string $heure):string {
    
    $h = (int)substr($heure, 0, 2);
    $m = substr($heure, 3, 2);
    if (! wa_est_entier($m)){ //$heure est une chaîne provenant de la BdD, donc méfiance
        $m = '00';
    }
    return "{$h}h{$m}mn";
}
...
*/
function wa_html_a(string $url, string $support_lien, ?string $query_string_nom = null, ?string $query_string_val = null, ?string $title = null):string{
 ...
 */
function wa_est_entier(mixed $x):bool {
 ...
 */
function wa_html_proteger_sortie(array|string $content): array|string {
    if (is_array($content)) {
        foreach ($content as &$value) {
            if (is_array($value) || is_string($value)){
                $value = wa_html_proteger_sortie($value);
            }
        }
...
*  @return   array|string               la chaîne protégée ou le tableau
*/  
function wa_bd_proteger_entree(mysqli $bd, array|string $content): array|string {
    if (is_array($content)) {
        foreach ($content as &$value) {
            if (is_array($value) || is_string($value)){
                $value = wa_bd_proteger_entree($bd,$value);
            }
        }
        unset ($value); // à ne pas oublier (de façon générale)
        return $content;
...
*/
function wa_parametres_controle(string $tab_global, array $cles_obligatoires, array $cles_facultatives = array()): bool{
 ...
 */
function wa_aff_ligne_input(string $libelle, array $attributs = array(), string $prefix_id = 'text'): void{
    echo    '<tr>', 
